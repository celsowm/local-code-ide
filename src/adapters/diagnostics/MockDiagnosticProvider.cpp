...
std::vector<Diagnostic> MockDiagnosticProvider::analyze(const QString&, const QString& text) {
    ...
    for (int i = 0; i < lines.size(); ++i) {
        ...
        if (line.contains("unsafe")) {
            diagnostics.push_back(makeDiag(i + 1, line.indexOf("unsafe") + 1, "Uso suspeito detectado por provider mock.", Diagnostic::Severity::Info));
        }
    ...
    if (diagnostics.empty()) {
        diagnostics.push_back(makeDiag(1, 1, "Nenhum problema crítico encontrado pelo provider mock.", Diagnostic::Severity::Info));
    }
